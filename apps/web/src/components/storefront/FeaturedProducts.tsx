    ...
    <div className="mt-10 grid gap-6 lg:grid-cols-3">
      {featured.map((product, index) => {
        ...
        return (
          ...
          >
            <div className={`relative h-28 bg-gradient-to-br ${tint}`}>
              <div className="absolute left-6 top-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-white text-sky-700 shadow-sm">
                <ShoppingCart className="h-6 w-6" />
              </div>
            </div>
            <div className="space-y-4 px-6 py-6">
              <div>
                <h3 className="text-[22px] font-semibold leading-tight tracking-[-0.04em] text-slate-950">
                  {product.name}
                </h3>
              ...
              <div className="flex items-end justify-between gap-4">
                <p className="text-[28px] font-semibold tracking-[-0.04em] text-sky-900">
                  {price != null ? `Từ ${formatVnd(price)}` : "Liên hệ"}
                </p>
                <Link
                  href={`/products/${product.slug}`}
                  className="flex h-11 w-11 items-center justify-center rounded-2xl border border-sky-100 bg-sky-50 text-sky-700 transition hover:bg-sky-100"
                >
